...
crawler = NaverNewsCrawler(input('키워드를 입력하시오: '))

#### 수집한 데이터를 저장할 엑셀 파일명을 input을 이용해 입력받아 ? 부분에 넣으세요
excel_file = input('엑셀 파일명을 입력하시오 :')+".xlsx"
...
SMTP_PORT = 465
SMTP_USER = input('구글 계정 입력:')
SMTP_PASSWORD = input('구글 비밀번호 입력: ')
...
ws.append([1,'name1','name1@gmail.com'])
ws.append([2,'name2','name2@naver.com'])
...
for row in data.rows:
  ...
  email_value.append(row_value)

subject = 'mini project3 입니다.'
contents = ' mini project3'
for i in range(1,len(email_value)):
      send_mail(email_value[i][1],email_value[i][2],subject, contents,excel_file)
